...
def insert_or_update_group_profile(yaml_path: Path, course_id: str, memory_requested: str):
    ...
    for i in range(custom_start + 1, jupyterhub_end):
        ...
        if indent == custom_indent and stripped and not stripped.startswith("#"):
            # Next top-level key inside custom
            break
    ...
    if group_profiles_start is None:
        ...
        jupyterhub_end += 1  # Adjust if used later
...
def main():
    ...
    if not hub_name or not course_id or not mem_requested:
    ...
    for c_id in re.split(r"[,\s:;]+", course_id):
        if c_id:  # skip empty strings
            insert_or_update_group_profile(yaml_path, c_id, convert_memory_to_interger(mem_requested))
